...
const components = () => {
  return {
    ...
    p: ({ children }: any) => (
      <Text mb={8} lineHeight="1.75">
        {children}
      </Text>
    ),
    h2: ({ children }: any) => (
      <Heading as="h2" fontSize="2xl" lineHeight="base" mt={16} mb={8}>
        {children}
      </Heading>
    ),
    h3: ({ children }: any) => (
      <Heading as="h3" fontSize="xl" lineHeight="base" mt={16} mb={8}>
        {children}
      </Heading>
    ),
    h4: ({ children }: any) => (
...
export const PostContent = ({ post }: PostContentProps) => (
  <PostContentLayout
    post={{ title: post.title.text.content, formattedDate: post.formattedDate }}
  >
